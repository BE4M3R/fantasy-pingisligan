-- Keep the locked squad snapshot as the audit record of the submitted lineup.
-- When its captain is automatically replaced, derive captaincy for the paired
-- incoming bench player while calculating the effective lineup and team total.
...
-- Correct already imported gameweeks during deployment. This is idempotent and
-- uses only persisted snapshots and results; no upstream request is involved.
do $$
declare
