-- Migration-history baseline created from the repository's complete schema.
--
-- This file builds a brand-new local database. The existing staging and
-- production projects already contain this schema and must mark version
-- 20260906000000 as applied after their schemas have been compared with this
-- baseline. Do not execute this entire file against either existing project.
...
begin
  ...
  select
    ...
    (
      ...
      0
    ) + scored_lineup.transfer_penalty_points)::integer as points,
    now(),
    now()
  from scored_lineup
  group by
...
-- Allow result mode to select any gameweek snapshot owned by the current user.

begin;

create or replace function public.get_my_squad_result(target_gameweek_id uuid)
returns table (
  gameweek_id uuid,
  gameweek_name text,
  round_order integer,
...
as $$
  ...
  fixture_bonuses as (
    ...
    join public.players
      on players.id = snapshot_players.player_id
    join public.matches
      on matches.fantasy_gameweek_id = selected_snapshot.fantasy_gameweek_id
      and lower(matches.status) = 'scored'
      and (
        (
          matches.winning_team_stupa_participant_id = matches.home_team_stupa_participant_id
  ...
  player_scores as (
    select
      selected_snapshot.fantasy_team_id,
      selected_snapshot.fantasy_gameweek_id,
      selected_snapshot.active_chip,
      selected_snapshot.gameweek_name,
      selected_snapshot.round_order,
      snapshot_players.player_id,
      snapshot_players.player_first_name_at_lock,
      snapshot_players.player_last_name_at_lock,
      snapshot_players.club_id_at_lock,
...
create or replace function public.get_my_squad_set_breakdown(target_gameweek_id uuid)
returns table (
  player_id uuid,
  singles_sets_won integer,
  singles_sets_lost integer,
...
revoke all on function public.get_my_squad_set_breakdown(uuid) from public;
grant execute on function public.get_my_squad_set_breakdown(uuid)
to authenticated;
