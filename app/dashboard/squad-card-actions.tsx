  ...
  return (
    <>
      <button
        ...
        className="group min-h-28 w-full rounded-md border border-white/15 bg-sky-950/70 p-4 text-left shadow-sm shadow-slate-950/20 transition hover:border-sky-100/60 hover:bg-sky-900/70 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-100"
      ...
      <dialog
        ...
        className="m-auto w-[min(28rem,calc(100%-2rem))] rounded-xl bg-transparent p-0 text-white backdrop:bg-slate-950/75"
        onClick={(event) => {
          if (event.target === event.currentTarget) event.currentTarget.close();
        }}
        ref={dialogRef}
      >
        <div className="rounded-xl border border-white/15 bg-sky-950 p-5 shadow-2xl sm:p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <button aria-label="Close player actions" className="rounded-md px-3 py-1 text-2xl text-sky-100/60 hover:bg-white/10 hover:text-white" onClick={() => dialogRef.current?.close()} type="button">×</button>
          </div>
